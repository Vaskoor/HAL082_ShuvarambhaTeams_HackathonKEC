<x-app-layout>
    ...
    <main class="p-4 lg:p-6 view-section active" id="user-management-view">
        ...
        <div class="card overflow-hidden">
            ...
            <script>
                let dusers = @json($users);
                let users = dusers.data.map((user, index) => ({
                    id: user.id,
                    name: user.name,
                    email: user.email,
                    role: index === 0 ? "Admin" : "Subscriber", // example assignment
                    status: "Active",                            // default status
                    avatar: user.name.split(" ")[0].toLowerCase() // first name as avatar
                }));
